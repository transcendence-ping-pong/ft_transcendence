...
export function renderGame(containerId: string) {
  ...
  if (container) {
    ...
    function showModalTournamentConfig() {
      const container = document.getElementById(containerId);
      if (container) {
        container.insertAdjacentHTML('beforeend', `
          <generic-modal dismissible="true" appear-delay="500">
            <create-tournament slot="body"></create-tournament>
          </generic-modal>
        `);
      }
    }

    window.addEventListener('openTournamentConfig', (e: CustomEvent) => {
      // only show modal if event is triggered
      showModalTournamentConfig();
    });
  }
}
