  ...
  container.innerHTML = `
    ...
    <form id="tournamentForm" class="flex flex-col gap-2">
      ${Array.from({ length: 8 }, (_, i) => `
        <input name="player${i}" placeholder="player${i + 1}" required class="input" />
      `).join('')}
      <button type="submit" class="btn btn-primary mt-2">Begin Tournament</button>
    </form>
    <div id="bracket"></div>
  `;
...
export async function renderTournamentBracket(tournamentId: number, bracketContainer: HTMLElement | null) {
  ...
  bracketContainer.innerHTML = `
    <h3 class="text-lg font-bold mb-2">Quarter Finals</h3>
    ${quarterfinals.map((match, idx) => `
      <div class="border p-2 my-1 rounded shadow">
        ...
        ${match.winnerDisplayName && match.winnerDisplayName.trim() !== ''
          ? `<p class="mt-1 text-green-600">🏆 Winner: ${match.winnerDisplayName}</p>`
          : nextMatch && match.matchId === nextMatch.matchId
            ? `<button class="btn btn-primary start-match-btn mt-2" data-matchid="${match.matchId}">Begin</button>`
            : ''
        }
      </div>
    `).join('')}
    ${quarterfinals.every(m => m.winnerDisplayName && m.winnerDisplayName.trim() !== '')
      ? `<p class="mt-4 font-semibold"Quarter Finals Concluded!</p>`
      : ''}
  `;
