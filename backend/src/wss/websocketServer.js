...
class WebSocketServer {
	...
	async handleDirectMessage(socket, data) {
		...
		// presence requirement: receiver must be on Home for PMs for now
		if (receiver.currentPath && receiver.currentPath !== '/') {
			socket.emit('chatError', { message: `${receiverUsername} is not on Home. Try again when they are on Home.` });
			return;
		}
